...
let dataFilterUpcoming = DataPrincipal.filter(
...
function FunRenderFinalForEvent(target, param) {
  if (target.checked) {
    ContenedorCardJS.innerHTML = ``;
    let VarFilter = Filter(dataFilterUpcoming, param);
...
VarInputSearch.addEventListener("input", (event) => {
  if (eventosFiltrados.length == 0) {
    let dataFilter = FilterName(dataFilterUpcoming, event.target.value);
